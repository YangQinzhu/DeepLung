...
class Loss(nn.Module):
    ...
    def forward(self, output, labels, train = True):
        ...
        pos_labels = pos_labels.to(torch.float32)
        pos_output = pos_output.to(torch.float32)
        ...
        if len(pos_output)>0:
            ...
            neg_prob = neg_prob.to(torch.float32)          # calculate BCEloss must be the float variable
            neg_labels = neg_labels.to(torch.float32)
