...
class DataBowl3Detector(Dataset):
    def __init__(self, data_dir, split_path, config, phase='train', split_comber=None):
        ...
        for idx in idcs:
            # print data_dir, idx
            # labelPath = data_dir+idx+'_label.npy'
...
class Crop(object):
    ...
    def __call__(self, imgs, target, bboxes,isScale=False,isRand=False):
        ...
        # print(type(normstart[0]), type(normstart[0]+normsize[0]), type(self.crop_size[0]/self.stride))
...
class LabelMapping(object):
    ...
    def __call__(self, input_size, target, bboxes, filename):
        ...
        # print(np.ones(output_size + [float(len(anchors)), 5.]))
        label = -1 * np.ones(output_size + [len(anchors), 5], np.int)

        #print(np.ones(output_size + [len(anchors), 5], np.int))
        # label = -1 * np.ones(output_size + [len(anchors), 5], np.int)
