...
savepath = '/data/yangqinzhu/ctLung/luna16/cls/crop_v3/'
annotationdetclsconvfnl_v3 = '/data/yangqinzhu/ctLung/DeepLung-master/nodcls/data/annotationdetclsconvfnl_v3.csv'
annotationdetclsconv_v3_csv = '/data/yangqinzhu/ctLung/DeepLung-master/nodcls/data/annotationdetclsconv_v3.csv'
SUBSET1_PATH = '/data/yangqinzhu/ctLung/luna16/subset1/'
########################################

pdframe =  pd.read_csv(annotationdetclsconv_v3_csv, names=['seriesuid', 'coordX', 'coordY', 'coordZ', 'diameter_mm', 'malignant'])
...
newlst = []
import csv
fid = open(annotationdetclsconvfnl_v3, 'w')
...
for idx in range(len(newlst)):
	...
	cropdata[int(CROPSIZE/2-cropdatatmp.shape[0]/2):int(CROPSIZE/2-cropdatatmp.shape[0]/2+cropdatatmp.shape[0]), \
	    int(CROPSIZE/2-cropdatatmp.shape[1]/2):int(CROPSIZE/2-cropdatatmp.shape[1]/2+cropdatatmp.shape[1]), \
	    int(CROPSIZE/2-cropdatatmp.shape[2]/2):int(CROPSIZE/2-cropdatatmp.shape[2]/2+cropdatatmp.shape[2])] = np.array(2-cropdatatmp)
